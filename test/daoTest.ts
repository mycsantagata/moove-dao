...
describe("MooveDao Test", function () {
  ...
  it("Purchase shares correctly and create new members", async function () {
    ...
    // Effettua l'acquisto di azioni da addr1
    await mooveDao.connect(addr1).buyShares(5);
    ...
    expect(await mooveDao.connect(addr1).getBalance()).to.equal(5000);
  });
  ...
  it("Close sale for members", async function () {
    ...
    await expect(mooveDao.connect(addr1).toggleSale()).to.be.revertedWith("Access denied!");
  });
  ...
  it("Exceed the supply limit", async function () {
    const {mooveDao} = await loadFixture(setupFixture);
    const addresses = await ethers.getSigners();
    const addressesToTest = addresses.slice(1, 18); // Recupero i primi 17 indirizzi (escludendo l'owner)
    const addressExpect = addresses[19];
    
    for(const address of addressesToTest){
        await mooveDao.connect(address).buyShares(10);
    }

    await expect(mooveDao.connect(addressExpect).buyShares(10)).to.be.revertedWith("Supply limit exceeded.");
  });
  ...
  it("Handle voting for proposed decisions correctly", async function () {
    const {mooveDao, addr1, addr2} = await loadFixture(setupFixture);
    await mooveDao.connect(addr1).buyShares(10);
    await mooveDao.connect(addr2).buyShares(5);
    await mooveDao.toggleSale();
    await mooveDao.connect(addr1).createProposal("Test Proposal");
  ...
  it("Approve Proposal", async function () {
    ...
    await mooveDao.connect(addr1).buyShares(10);
    await mooveDao.connect(addr2).buyShares(1);
    await mooveDao.toggleSale();
    await mooveDao.connect(addr1).createProposal("Test Proposal");
    await mooveDao.connect(addr1).voteProposal(0, 0);
    await mooveDao.connect(addr2).voteProposal(1, 0);   
  ...
  it("Reject Proposal", async function () {
    ...
    await mooveDao.connect(addr1).buyShares(1);
    await mooveDao.connect(addr2).buyShares(10);
    await mooveDao.toggleSale();
    await mooveDao.connect(addr1).createProposal("Test Proposal");
    await mooveDao.connect(addr1).voteProposal(0, 0);
    await mooveDao.connect(addr2).voteProposal(1, 0);   
